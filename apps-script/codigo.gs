...
function checkTimestamp() {
  try {
    ...
    var timestamp = data.lastUpdate || null;
    
    return ContentService
      .createTextOutput(JSON.stringify({timestamp: timestamp, empty: false}))
      .setMimeType(ContentService.MimeType.JSON);
  } catch(err) {
    return ContentService
      .createTextOutput(JSON.stringify({error: err.message}))
...
function saveAll(data, forceWrite) {
  ...
  try {
    ...
    // Lê dados atuais do servidor para verificar timestamp
    var raw = dbSheet.getRange('A1').getDisplayValue();
    var jsonValue = (raw === null || raw === undefined) ? '' : String(raw);
    jsonValue = jsonValue.trim();
    ...
    // Se não for forcing e houver timestamps, verifica quem é mais recente
    var needsMerge = false;
    var mergedData = null;
    
    if (!forceWrite && serverTimestamp && clientTimestamp) {
      // Compara timestamps
      if (serverTimestamp !== clientTimestamp) {
        // Timestamps diferentes - precisa mesclar
        mergedData = mergeData(serverData, data);
        needsMerge = true;
      }
    }
    
    // Se não precisa merge ou é force write, usa dados do cliente
    if (!needsMerge) {
      // Adiciona timestamp atual
      data.lastUpdate = new Date().toISOString();
      
      // Garante que nxt é o maior entre local e servidor
      ...
      // Se há dados no servidor que não estão nos dados locais, preserva
      if (serverData) {
        // Preserva vendas do servidor que não existem localmente
        if (serverData.vendas && data.vendas) {
          ...
          serverData.vendas.forEach(function(v) {
            if (localVendasIds.indexOf(v.id) === -1) {
              data.vendas.push(v);
            }
          });
        }
        
        // Preserva produtos do servidor que não existem localmente
        if (serverData.produtos && data.produtos) {
          ...
          serverData.produtos.forEach(function(p) {
            if (localProdutosIds.indexOf(p.id) === -1) {
              data.produtos.push(p);
            }
          });
        }
        
        // Preserva vendedores do servidor que não existem localmente
        if (serverData.vendedores && data.vendedores) {
          ...
          serverData.vendedores.forEach(function(v) {
            if (localVendedoresNomes.indexOf(v.nome) === -1) {
              data.vendedores.push(v);
            }
          });
        }
      }
    } else {
      // Usa dados mesclados e adiciona timestamp
    ...
    // Sincroniza com as abas auxiliares
...
function mergeData(serverData, clientData) {
  ...
  // NXT - usa o maior
  merged.nxt = Math.max(
    (serverData && serverData.nxt) || 0,
    (clientData && clientData.nxt) || 0
  );
  
  // === VENDEDORES ===
  var vendedorMap = {};
  
  if (serverData && serverData.vendedores) {
    serverData.vendedores.forEach(function(v) {
      vendedorMap[v.nome] = v;
    });
  }
  
  if (clientData && clientData.vendedores) {
    clientData.vendedores.forEach(function(v) {
      vendedorMap[v.nome] = v;
    });
  }
  ...
  if (serverData && serverData.produtos) {
    serverData.produtos.forEach(function(p) {
      produtoMap[p.id] = p;
    });
  }
  
  if (clientData && clientData.produtos) {
    clientData.produtos.forEach(function(p) {
      produtoMap[p.id] = p;
    });
  }
  ...
  if (serverData && serverData.vendas) {
    serverData.vendas.forEach(function(v) {
      vendaMap[v.id] = v;
    });
  }
  
  if (clientData && clientData.vendas) {
    clientData.vendas.forEach(function(v) {
      vendaMap[v.id] = v;
    });
  }
...
function syncProdutos(ss, produtos) {
  ...
  produtos.forEach(function(p) {
    rows.push([
      p.id || '',
      p.nome || '',
      p.custo !== undefined ? p.custo : '',
      p.preco !== undefined ? p.preco : '',
      p.est !== undefined ? p.est : '',
      p.ean || '',
      p.comissao !== undefined ? p.comissao : ''
    ]);
  });
...
function syncVendas(ss, vendas) {
  ...
  vendas.forEach(function(v) {
    rows.push([
      v.id || '',
      v.grupoId !== undefined ? v.grupoId : '',
      v.vendedor || '',
      v.produto || '',
      v.preco !== undefined ? v.preco : '',
      v.quantidade !== undefined ? v.quantidade : '',
